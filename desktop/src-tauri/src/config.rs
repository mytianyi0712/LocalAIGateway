...
const fn default_port() -> u16 { DEFAULT_PORT }

impl Default for AppConfig {
    fn default() -> Self {
        Self { host: default_host(), port: DEFAULT_PORT, data_dir: PathBuf::from("data") }
    }
}
...
impl AppConfig {
    pub async fn load(platform_data_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        if let Ok(host) = env::var("AI_GATEWAY_HOST") { config.host = host; }
    ...
    pub fn database_path(&self) -> PathBuf { self.data_dir.join("gateway.db") }
    pub fn key_path(&self) -> PathBuf { self.data_dir.join("master.key") }
    pub fn local_url(&self) -> String { format!("http://127.0.0.1:{}", self.port) }
}

fn resolve_data_dir(platform_data_dir: &Path) -> PathBuf {
    if let Some(value) = env::var_os("AI_GATEWAY_DATA_DIR") { return PathBuf::from(value); }
    let legacy = PathBuf::from("data");
    if legacy.join("gateway.db").exists() || legacy.join("master.key").exists() { legacy } else { platform_data_dir.to_path_buf() }
}

pub fn validate_port(port: u16) -> Result<u16> {
    if port < 1024 { bail!("端口必须在 1024–65535 之间"); }
    Ok(port)
}
